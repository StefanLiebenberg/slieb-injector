...
slieb.injector.providers.DisposableSingletonProvider = function(provider) {
    ...
    slieb.injector.providers.DisposableSingletonProvider.base(this,
      'constructor');
    this.provider = provider;
};
goog.inherits(slieb.injector.providers.DisposableSingletonProvider,
  goog.Disposable);

/**
 * @return {T}
 */
slieb.injector.providers.DisposableSingletonProvider.prototype.get =
  function() {
    if (!goog.isDefAndNotNull(this.instance) ||
      (/** @type {goog.Disposable} */ (this.instance)).isDisposed()) {
        /** @type {T} */
